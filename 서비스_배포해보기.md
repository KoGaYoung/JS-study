...
jobs:
  deploy:
    ...
    steps:
      # GitHub 리포지토리를 체크아웃합니다.
      - name: Checkout Repository
        uses: actions/checkout@v3

      # Node.js를 설정합니다. 버전 18을 사용합니다.
      - name: Set up Node.js
      ...
      - name: Deploy to Server
        env:
          ...
          SERVER_IP: ${{ secrets.BUILD_SSH_IP }}
          SERVER_USER: ${{ secrets.BUILD_SSH_USER }}
        run: |
          # SSH 키 설정을 수행합니다.
          mkdir -p ~/.ssh
          ...
          # 빌드 서버에 SSH로 접근하여 빌드 스크립트를 실행합니다.
          # 여기서는 사용자 정의 스크립트 './start_backend.sh'를 실행합니다.
          ssh -i ~/.ssh/id_rsa $BUILD_SSH_USER@$BUILD_SSH_IP "./start_frontend.sh"
~~~
