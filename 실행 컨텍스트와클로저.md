...
useEffect(() => {
   const arrElements = document.getElementsByClassName("bithumb-mileage-animation-section");

   // 맨 처음에 이렇게 썼다. 
   // let position = new Array(); // 여기가 문제였던것..
   ...
   */
   const calcPosition = () => {
      var e = d.current.section
      , i = window.scrollY;

      x.current.length > e + 1 && x.current[e + 1] <= window.innerHeight + i && (r(e + 1),
      d.current.section += 1),
