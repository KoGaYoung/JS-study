...
~~~
결론부터 말하면 실행컨택스트(현재 실행 환경 정보)가 참조하는 객이다

this를 알기 위해서는 스코프와 실행컨택스트 사이의 관계를 알아야한다.

이전 글 동시성(Concurrency)에서 자바스크립트는 하나의 스레드로 한번에 한개의 작업만 할 수 있고,
이때 실행 컨택스트라고 불리는 스크립트(또는 함수)가 실행되는 동안 필요한 모든 정보를 담고있는 환경을 만듭니다.
실행 컨택스트가 만든 환경 중 현재 참조하는 객체가 this를 라고합니다
...
(지금 내가 사용할 수 있는 변수들, 함수들 모음집이라고 생각, 부분집합으로 this존재)
...
자바스크립트는 정적(렉시컬) 스코핑 때문에 선언되는 위치에 따라 상위 스코프가 결정되고, 함수가 외부함수 내에 존재할 경우 선언 될 때 당시의 값을 기억하고있기 때문에 외부자원을 사용할 수 있게하는 기능이라고 할 수 있다! (선언이라는 키워드에 빠져서 함수 선언식에서만 일어나고 표현식에서는 안 일어나나? 토끼굴에 빠질뻔 했다. 결론은 선언 표현 둘 다 클로저는 발생한다. 중요한건 어떻게 선언되었느냐가 아니라, 함수가 외부스코프 변수에 접근할 수 있는 환경이 생성되었는가입니다)
~~~
~~~
